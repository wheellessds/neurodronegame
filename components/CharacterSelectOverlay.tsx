...
export const CharacterSelectOverlay: React.FC<CharacterSelectOverlayProps> = ({ onSelect, onClose, isMobile }) => {
    ...
    return (
        <div
            className={`fixed inset-0 z-[1100] bg-slate-950 flex overflow-hidden font-sans select-none cursor-grab active:cursor-grabbing transition-all duration-1000 ease-out ${
        ...
        >
            ...
            <div className={`absolute inset-0 pointer-events-none z-10 overflow-hidden flex items-end justify-center lg:justify-end transition-all duration-1000 ease-out ${
                showBackground ? 'opacity-100 scale-100' : 'opacity-0 scale-95'
            }`}>
                    ...
                    return (
                        ...
                        >
                            ...
                            <img
                                src={char.image}
                                alt={char.name}
                                className={`h-[75vh] md:h-[95vh] lg:h-[115vh] w-auto max-w-none object-contain filter transition-all duration-1000 ${isActive ? 'animate-portrait' : ''}`}
                            />
            ...
            <div className={`relative z-30 w-full h-full flex flex-col ${isMobile ? 'px-6 pt-6 pb-2 justify-between' : 'p-6 md:p-12 lg:p-16'}`}>
                ...
                {/* Navigation Header */}
                <div className={`flex items-center justify-between ${isMobile ? 'mt-2 h-12' : 'mb-4 lg:mb-8'} transition-all duration-700 ease-out ${
                    ...
                    <div className="flex items-center gap-3 bg-slate-900/80 backdrop-blur-md px-4 py-2 rounded-full border border-slate-800 shadow-2xl">
                        ...
                        <div className="text-slate-500 font-mono text-xs w-10 text-center select-none"><span className="text-white font-bold">{selectedIndex + 1}</span> / 2</div>
                ...
                <div className={`flex-1 flex flex-col ${isMobile ? 'pt-4 pb-0 max-w-full' : 'justify-center max-w-2xl'} transition-all duration-700 ease-out ${
                    showContent ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'
                }`}>
                    {/* Header/Drone Group */}
                    <div className={isMobile ? 'flex flex-col' : ''}>
                        <div className={isMobile ? 'mb-4' : 'mb-6'}>
                            <span className={`text-cyan-500 block text-[10px] md:text-sm font-mono tracking-[0.3em] opacity-80 mb-1 transition-all duration-500 ease-out ${
                                showPilotLabel ? 'translate-x-0 opacity-80' : '-translate-x-4 opacity-0'
                            }`}>UNITS // PILOT-01</span>
                            <h2 className={`${isMobile ? 'text-4xl' : 'text-4xl md:text-5xl lg:text-7xl'} font-black text-white italic tracking-tighter leading-none uppercase transition-all duration-500 ease-out ${
                                showCharacterName ? 'translate-x-0 opacity-100' : '-translate-x-4 opacity-0'
                            }`}>
                                {currentChar.name}
                            </h2>
                            <div className={`${isMobile ? 'h-0.5 w-16' : 'h-1 lg:h-1.5 w-24'} mt-4 transition-all duration-500 ${
                                showCharacterName ? 'scale-x-100 opacity-100' : 'scale-x-0 opacity-0'
                    ...
                    <div className={`${isMobile ? 'mt-auto flex flex-col gap-6' : ''}`}>
                        {/* Specs Card */}
                        <div className={`${isMobile ? 'w-3/5 pl-4' : 'mb-8 pl-8'} border-l-4 py-3 bg-slate-900/60 backdrop-blur-md rounded-r-2xl transition-all duration-500 ease-out ${
                            showDescription ? 'translate-x-0 opacity-100' : '-translate-x-4 opacity-0'
                        ...
                        {/* Action Button */}
                        <div className={`${isMobile ? 'pb-2' : ''} transition-all duration-500 ease-out ${
                            showStartButton ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'
                        }`}>
                            <button
                                onClick={(e) => {
                                    e.stopPropagation();
                ...
                <div className={`${isMobile ? 'mt-2' : 'mt-8'} flex justify-between items-end opacity-20 font-mono text-[10px] tracking-widest uppercase select-none transition-all duration-700 ease-out ${
                    showFooter ? 'translate-y-0 opacity-20' : 'translate-y-4 opacity-0'
                }`}>
